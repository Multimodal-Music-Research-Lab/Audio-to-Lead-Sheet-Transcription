...
class Decoder(nn.Module):
    def __init__(
        ...
        # Transformer block
        # Reuse the largest causal mask during autoregressive inference.
    ...
    def forward(self, tgt, memory, memory_len):
        ...
        )  # hidden.shape = [batch_size, tgt_sec_len, emb_dim]
    ...
    def get_memory_key_padding_mask(self, memory, memory_len):
        ...
        # When using batches, the sequences are padded to the same length
    ...
    def get_tgt_masks(self, tgt):
        ...
        tgt_mask = self.causal_mask[:tgt_sec_len, :tgt_sec_len].to(tgt.device)
